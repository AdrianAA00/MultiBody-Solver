program main
  ...
  !Initial conditions
  real*8, parameter :: pi=acos(-1d0), psi0 = 0d0* (pi/180d0), theta0 = 90d0* (pi/180d0), phi0 = 0d0* (pi/180d0)

  !Numerical solution
  integer :: iter_max=10000, iter
  ...
  real*8, parameter :: l=1d0, I=1d0
  real*8, parameter :: g=9d0
  ...
  ! --------------------------------------------------------------------- !
  ! Analytical solution
  call analytical (theta0,iter_max,delta_t,g,l)
  ...
  do iter = 1, iter_max
    ...
    G_ = fG_ (q(4),q(5),q(6),q(7))
    A = fA (q(4),q(5),q(6),q(7))
    u = fu (x_p(1),x_p(2),x_p(3))
    constraint = q(1:3) + MATMUL(MATMUL(MATMUL(A,u),G_), q(4:7))
    write (print, *) q(2),q(3) !sum((constraint)**2)

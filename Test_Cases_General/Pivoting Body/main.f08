program main
  ...
  !Initial conditions
  real*8, parameter :: pi=acos(-1d0), psi0 = 0d0* (pi/180d0), theta0 = 45d0* (pi/180d0), phi0 = 0d0* (pi/180d0)

  !Numerical solution
  integer :: iter_max=4000, iter
  real*8 :: delta_t=0.001, gamma=0.5, beta=0.25, tau=0.99 !Relaxation factor. First order: gamma = 0.75 and beta = 0.390625

  real*8 :: M(11,11), b(11), aux(11,11), aux2(11)
  real*8 :: x(11), x_new(11)  !Solution to the system
  integer :: pivot(10)

  !Physical problem parameters
  real*8, parameter :: l=0d0, mass = 1d0, A_ = 1d0, B_ = 1d0, C_ = 2d0, D_ = 1d0, mu = 1d0
  ...
  real*8 :: m00(3,3)=reshape(    (/ A_, 0d0, 0d0, &
                                    0d0, B_, 0d0, &
                                    0d0, 0d0, C_ /), &
                                (/3,3/), order = (/2,1/) )

  !Forces and moments
  real*8 :: Q_e(7)=reshape(  (/ 0d0, 0d0, -g*mass, 0d0, 0d0, 0d0, 0d0/), (/7/) ) 
  ...
  !Initial conditions
  real*8 :: EuAng(3)=reshape(  (/  psi0 , theta0,  phi0 /), (/3/) )
  real*8 :: EuVel(3)=reshape(  (/ D_*mu/A_, 0d0, D_*mu*cos(theta0)*(A_/C_-1d0)/A_ /), (/3/) )
  ...
  ! --------------------------------------------------------------------- !
  ! Analytical solution
  call  analytical (iter_max,delta_t,psi0,theta0,phi0,A_,B_,C_,D_,mu)

  ! --------------------------------------------------------------------- !
  ! Verification of matrices and solver
  write(*,*) "Euler Initial Angles =", EuAng
  write(*,*) "Euler Initial Parameter =", e
  write(*,*) "Euler Initial Angles velocities=", EuVel
  write(*,*) "Euler Initial Parameter velocities =", de

  M = fM(x_p,e,mRR,m00)
  aux = M
  write(*,*) "________M_________"
  write(*,'(11F8.3)') Transpose(aux(:,:))
  !write(*,*) "Det =", DET(aux)
  
  b = fb(x_p,e,de,Q_e,ddc,m00,ddr_p)
  ...
  write(*,'(11F8.2)') Transpose(M(:,:))

  b = pivoting(b,pivot)
  call solve(M, b, x)

  write(*,*) "______verify solver______" 
  write(*,'(11F8.5)') MATMUL(aux,x) - aux2
  ...
  do iter = 1, iter_max
    ...
    do while( ( dq_res > TOLERANCE .or. q_res > TOLERANCE ) .and. iter2 < 100)
      ...
      q_temp(1:3) = q_temp(1:3)*l / sqrt(sum(q_temp(1:3)**2))
  ...
  subroutine analytical (N,delta_t,psi0,theta0,phi0,A,B,C,D,mu)
    implicit none
      integer, intent(in) :: N
      real*8, intent (in) :: delta_t,psi0,theta0,phi0,A,B,C,D,mu
      real*8 :: psi,theta, phi 
      integer :: i,fu1,fu2,fu3

      
      open (action='write', file="data_analytical_psi.txt", newunit=fu1, status='replace')
      open (action='write', file="data_analytical_theta.txt", newunit=fu2, status='replace')
      open (action='write', file="data_analytical_phi.txt", newunit=fu3, status='replace')

      do i=1,N
        psi=D*mu*i*delta_t/A + psi0
        theta=theta0
        phi=D*mu*i*cos(theta0)*(A/C-1d0)*delta_t/A + phi0
        write (fu1, *) i, psi
        write (fu2, *) i, theta
        write (fu3, *) i, phi

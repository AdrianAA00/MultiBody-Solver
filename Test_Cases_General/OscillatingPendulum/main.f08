program main
  ...
  !Initial conditions
  real*8, parameter :: pi=acos(-1d0), psi0 = 90d0* (pi/180d0), theta0 = 0.5d0* (pi/180d0), phi0 = 0d0* (pi/180d0)

  !Numerical solution
  integer :: iter_max=4000, iter
  real*8 :: delta_t=0.001, gamma=0.5, beta=0.25, tau=0.999 !Relaxation factor. First order: gamma = 0.75 and beta = 0.390625
  ...
  real*8, parameter :: l=1d0, mass = 13000d0, I=1d-1
  ...
  !Initial conditionsl*sin(theta0)
  real*8 :: EuAng(3)=reshape(  (/  psi0 , theta0,  phi0 /), (/3/) )
  real*8 :: EuVel(3)=reshape(  (/ -sqrt(abs(g*tan(theta0))/(l*sin(theta0))), 0d0, 0d0 /), (/3/) ) !-sqrt(abs(g*tan(theta0))/(l*sin(theta0)))
  ...
  dr = reshape((/ 0d0,sqrt(abs(g*l*sin(theta0)*tan(theta0))), 0d0/), (/3/) )
  q = 0d0
  dq = 0d0
  q(1:3) = r
  q(4:7) = e
  ...
  ! --------------------------------------------------------------------- !
  ! Analytical solution
  call  analytical (iter_max,delta_t,g,l,theta0)
  ...
  subroutine analytical (N,delta_t,g,l,theta0)
    implicit none
      integer, intent(in) :: N
      real*8, intent (in) :: g, l,theta0, delta_t
      real*8 :: x,y
      integer :: i,fu
      ...
      do i=1,N
        y=l*sin(theta0)*cos(i*delta_t*sqrt((g*tan(theta0))/(l*sin(theta0))))
        x=l*sin(theta0)*sin(i*delta_t*sqrt((g*tan(theta0))/(l*sin(theta0))))
        write (fu, *) x, y
      end do

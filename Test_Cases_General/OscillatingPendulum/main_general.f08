program main
  ...
  real*8, parameter :: beta = 0.25, gamma = 0.5, tau = 0.999   !Newmark parameters 
  ...
  write(*,'(11F8.5)') MATMUL(auxM,x_new) - b
  ...
  do iter = 1, iter_max
    ...
    do while( ( dq_res > TOLERANCE .or. q_res > TOLERANCE ) .and. iter2 < 100)
      ...
      ((1-gamma)*ddconstrains(3:,:) + gamma*ddconstrains_new(3:,:))
  ...
  subroutine position_correction (q_temp,indices,x_p,NumberBodies,q_constrains)
    implicit none
    ! Correcting for x,y,z position of each body based on the given contrain. 
    ! Finding new position of constrained body maintaining the fixed coordinate of the contrain reference point.
      ...
      real*8 ::  A(3,3)
      real*8, allocatable :: auxq(:,:) 
      integer :: index, count, count2, master_index, slave_index

      allocate(auxq(3,NumberBodies+1))
      auxq = 0d0

      ! Setting displacement "auxq" of each body with respect to its fixed reference point
      do count2 = 1, NumberBodies+1  !Body attached to fixed point index
        do count = 1, NumberBodies+1   !Fixed Point index
          if ((constrain_coupling(count,count2) == 1 .AND. count == count2)) then
            ...
            auxq(:,count2) = auxq(:,count2) + q_constrains(3:5,count2) - MATMUL(A,x_p(:,count,count))  !Diagonal
          elseif ((constrain_coupling(count,count2) == 1 .AND. count /= count2)) then
            ...
            auxq(:,count2) = auxq(:,count2) + MATMUL(A,x_p(:,count,count2))  !Diagonal !Off-diagonal
          end if 
        end do
      end do

      ! Accumulating displacements of several master-slave-slave-etc elements
      do count2 = 1, NumberBodies  !Select slave element        
        master_index = int(indices(1,count2))
        slave_index = int(indices(2,count2))
        if (slave_index /= count2) then
          write(*,*) "Indexes of slave elements not properly ordered. Please define elements &
          in input file in order, start from 1st body to NBody."
          STOP
        end if
        do while (slave_index > 0)  !Go through all the masters above that slave that define its position. Inertial reference frame 0 is not moving, do not select 0.
          master_index = int(indices(1,slave_index))
          slave_index = int(indices(2,slave_index))
          if (master_index /= slave_index) then
            auxq(:,slave_index+1) = auxq(:,master_index+1) + auxq(:,slave_index+1)
            slave_index = master_index
          else
            EXIT  !Go out in elements constrained to themselves
          end if
        end do
      end do

      ! Substitute constrains in the component where the element is constraint
      do count2 = 1, NumberBodies  !Select slave element  
        do index = 1,3
          if (indices(2+index,count2) == 1) then
            q_temp(1+index,count2+1) = auxq(index,count2+1)
          end if
        end do
      end do
  ...
  subroutine inertial_general_F (Q_g,F_inert,e)
    implicit none
    ! Converting inertial forces and moments to generalized components. 
  ...
  subroutine read_iter(Q_g, ddconstrains_new, NumberBodies,q)
    ...
    real*8, allocatable :: F_inert(:,:), ddconstrains_temp(:,:)
    integer, intent(in) :: NumberBodies
    ...
    allocate(ddconstrains_new(9,NumberBodies))
    allocate(ddconstrains_temp(8,NumberBodies))
    ...
    do iter_read = 0, max_iter_read
      ...
      if (ierr == 0) then
        call inertial_general_F (Q_g(:,count),F_inert(:,count),q(5:8,count+1))      ! Converting to generalized forces
    ...
    count = 1
    do iter_read = 0, max_iter_read
      read (0,*,iostat=ierr) ddconstrains_temp(:,count)
      if (ierr == 0) then
        call inertial_general_F (ddconstrains_new(2:,count),ddconstrains_temp(2:,count),q(5:8,count+1))      ! Converting to accelerations in Euler parameters
        ddconstrains_new(1:2,count) = ddconstrains_temp(1:2,count) ! Index of slave and master
